...
pub fn start_discovery(peer_id: PeerId, registry: PeerRegistry) -> anyhow::Result<()> {
    ...
    let service_name = format!("{}.{}", peer_id.to_string(), SERVICE_TYPE);
    let my_info = ServiceInfo::new(SERVICE_TYPE, &peer_id.to_string(), &service_name, "", 9000, None)?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            let _ = tx_socket.send_to(msg.as_bytes(), format!("255.255.255.255:{}", BROADCAST_PORT));
